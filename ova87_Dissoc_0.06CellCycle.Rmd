...
```
## In this Rmd all samples from patioent OVA87 will be combined
...
cc.genes <- readLines(con = "/path/to/data/data/cellCycleMarkers/regev_lab_cell_cycle_genes.txt")
...
# Read the datasets one by one
ovary.1.data <- Read10X(data.dir = "/path/to/data/cellRangerOutput/sample1_B1_i12A/outs/filtered_gene_bc_matrices/GRCh38/")
ovary.2.data <- Read10X(data.dir = "/path/to/data/cellRangerOutput/sample2_B1_i12B/outs/filtered_gene_bc_matrices/GRCh38/")
ovary.3.data <- Read10X(data.dir = "/path/to/data/cellRangerOutput/sample3_B1_i12G/outs/filtered_gene_bc_matrices/GRCh38/")
ovary.4.data <- Read10X(data.dir = "/path/to/data/cellRangerOutput/sample4_B1_i12F/outs/filtered_gene_bc_matrices/GRCh38/")
ovary.5.data <- Read10X(data.dir = "/path/to/data/cellRangerOutput/sample5_B2_i10E/outs/filtered_gene_bc_matrices/GRCh38/")
ovary.6a.data <- Read10X(data.dir = "/path/to/data/cellRangerOutput/sample6a_B1_i12H/outs/filtered_gene_bc_matrices/GRCh38/")
ovary.7.data <- Read10X(data.dir = "/path/to/data/cellRangerOutput/sample7_B2_i10C/outs/filtered_gene_bc_matrices/GRCh38/")
ovary.8a.data <- Read10X(data.dir = "/path/to/data/cellRangerOutput/sample8a_B2_i10A/outs/filtered_gene_bc_matrices/GRCh38/")
ovary.8b.data <- Read10X(data.dir = "/path/to/data/cellRangerOutput/sample8b_B2_i10B/outs/filtered_gene_bc_matrices/GRCh38/")
ovary.10.data <- Read10X(data.dir = "/path/to/data/cellRangerOutput/sample10_B2_i10D/outs/filtered_gene_bc_matrices/GRCh38/")
ovary.145.data <- Read10X(data.dir = "/path/to/data/cellRangerOutput/sample145_B1_i12D/outs/filtered_gene_bc_matrices/GRCh38/")
ovary.C1.data <- Read10X(data.dir = "/path/to/data/cellRangerOutput/sampleC1_B1_i12E/outs/filtered_gene_bc_matrices/GRCh38/")
```
...
## This list of genes is derived from the supl. materials of https://www.nature.com/articles/nmeth.4437 
...
We also print the % Variance explained by each of the PCs

```{r elbow_plot, fig.height=6, fig.width=10, warning=FALSE}
PCElbowPlot(object = ovary)

## Calculate the % of variance explained by each indivisual PC
## Divide each of the sdev by the sum of the first 20 sdevs
PCAsdev <- ovary@dr$pca@sdev
names(PCAsdev) <- colnames(ovary@dr$pca@gene.loadings)
sumSdev <- sum(PCAsdev)
cat("## This is the % of variance explained by each of the PCs")
print(PCAsdev/sumSdev*100)
```
...
ovary <- FindClusters(object = ovary, reduction.type = "pca", dims.use = 1:12, 
...
PCAPlot(object = ovary, dim.1 = 1, dim.2 = 2, pt.size=0.2)
PCAPlot(object = ovary, dim.1 = 1, dim.2 = 2, pt.size=0.2, group.by = 'orig.ident')
...
save(ovary, file =  ovaryRds)
```



## Plot the epsression of some of the (alleged) dissociation-induced genes  across clusters
```{r dissociationGenes, fig.height=8, fig.width=9}

FeaturePlot(object = ovary, features.plot = c("DNAJB1","HSPA1A","DNAJA1","GADD45B","HES1","JUN","PPP1R15A","HSPB1"), 
            cols.use = c("grey", "blue"), reduction.use = "tsne")
```


### As an extra, plot some interactive tSNEs
```{r interactivetSNEs, fig.height=8, fig.width=9}
TSNEPlot(object = ovary, do.label = T, do.hover = TRUE, do.identify = TRUE)
TSNEPlot(object = ovary, do.label = T, group.by = "orig.ident", do.hover = TRUE, do.identify = TRUE)
```
